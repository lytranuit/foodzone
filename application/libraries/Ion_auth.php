...
class Ion_auth
{
    ...
    {    //changed $email to $identity
        if ($this->ion_auth_model->forgotten_password($identity)) {   //changed
            ...
            $user = $this->where($identifier, $identity)->where('active', 1)->where('deleted', 0)->users()->row();  // changed to get_user_by_identity from email
    ...
    { //need to test email activation
        ...
        if (!$email_activation) {
            ...
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
                $activate = $this->ion_auth_model->activate($id);
